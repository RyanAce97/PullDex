...
export function Collection() {
  ...
  if (error) {
    return <ErrorState message="Failed to load collection data." />;
  }
  ...
  if (data.length === 0) {
    return (
      <EmptyState
        icon="📋"
        message="No species data yet. Import data to get started."
      />
    );
  }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {filtered.map((s) => (
              <CollectionSpeciesCard
                key={s.species_id}
                species={s}
                onClick={() => navigate(`/pokedex/${s.species_id}`)}
              />
            ))}
          </div>
        </>
...
function CollectionSpeciesCard({ species, onClick }: CollectionSpeciesCardProps) {
  return (
    <div
      className={`flex items-center gap-3 rounded-lg border p-3 transition-colors cursor-pointer hover:shadow-md hover:border-indigo-300 ${
        species.owned
